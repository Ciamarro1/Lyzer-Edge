...
You are Reviewer 3 for Milestone 2 Iteration 2 (R2: Asynchronous Batching in SQLite db.js).
...
1. Verify `this._flushPromise.catch(() => {});` in `lyzer edge/backend/db.js` prevents unhandled rejections on rollback/error while preserving error propagation to callers.
2. Verify test isolation in `lyzer edge/tests/causal-memory/causalBatching.test.js`.
