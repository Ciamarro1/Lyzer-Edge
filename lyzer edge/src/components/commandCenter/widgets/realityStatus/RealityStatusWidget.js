...
export class RealityStatusWidget {
    ...
    this._container.innerHTML = `
      <div style="padding: 14px; border: 1px solid rgba(56,189,248,0.12); border-radius: 10px; font-family: 'JetBrains Mono', monospace; background: rgba(8,12,20,0.5); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); color: #f1f5f9; font-size: 11px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 10px; background: rgba(15,23,42,0.4); padding: 8px; border-radius: 6px; border: 1px solid rgba(148,163,184,0.06);">
          ...
          <div>EEF Constraint: <span id="rs-eef" style="color: #64748b; font-weight: bold;">--</span></div>
        </div>
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; background: rgba(15,23,42,0.6); padding: 8px; border-radius: 6px; text-align: center; border: 1px solid rgba(56,189,248,0.1);">
          <div>
            ...
            <div id="rs-dvf" style="font-weight: 800; color: #38bdf8; font-size: 13px;">--</div>
          </div>
          <div>
            <div style="font-size: 8px; color: #94a3b8; text-transform: uppercase;">LHDS</div>
            <div id="rs-lhds" style="font-weight: 800; color: #f43f5e; font-size: 13px;">--</div>
          </div>
          <div>
            <div style="font-size: 8px; color: #94a3b8; text-transform: uppercase;">CONF</div>
            <div id="rs-conf" style="font-weight: 800; color: #a855f7; font-size: 13px;">--</div>
          </div>
          <div>
            <div style="font-size: 8px; color: #94a3b8; text-transform: uppercase;">SDS</div>
            <div id="rs-sds" style="font-weight: 800; color: #fbbf24; font-size: 13px;">--</div>
          </div>
        </div>
      </div>
    `;
  ...
  _updateUI() {
    try {
      ...
      this._uiElements.mode.textContent = status.realityTag || '--';
      this._uiElements.provider.textContent = status.providerId || 'live-binance-v4';
      this._uiElements.health.textContent = status.healthStatus || 'AWAITING_STREAM';
      this._uiElements.latency.textContent = status.latencyMs !== undefined ? status.latencyMs : '--';
      
      if (this._uiElements.eef) {
        if (status.eef === undefined || status.eef === null) {
          this._uiElements.eef.textContent = '--';
          this._uiElements.eef.style.color = '#64748b';
        } else {
          const isAllow = (status.eef === true || status.eef === 'ALLOW' || status.eef === 'ALLOW_TRANSITION');
          this._uiElements.eef.textContent = isAllow ? 'VALID' : 'VETOED';
          this._uiElements.eef.style.color = isAllow ? '#4ade80' : '#ef4444';
        }
      }
      if (this._uiElements.dvf) {
        this._uiElements.dvf.textContent = status.dvf !== undefined && status.dvf !== null ? Number(status.dvf).toFixed(2) : '--';
      }
      if (this._uiElements.lhds) {
        this._uiElements.lhds.textContent = status.lhds !== undefined && status.lhds !== null ? Number(status.lhds).toFixed(3) : '--';
      }
      if (this._uiElements.conf) {
        this._uiElements.conf.textContent = status.conf !== undefined && status.conf !== null ? Number(status.conf).toFixed(1) + '%' : '--';
      }
      if (this._uiElements.sds) {
        this._uiElements.sds.textContent = status.sds !== undefined && status.sds !== null ? Number(status.sds).toFixed(2) : '--';
      }
