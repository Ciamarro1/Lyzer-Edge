...
export async function closeTrade(id, exitData) {
  ...
  if (trade && trade.backendId) {
    try {
      ...
      console.error('[Queries] Failed to sync close with backend:', e);
    }
  }
  ...
  if (trade && trade.backendId) {
    try {
      await fetch('/api/trades/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ symbol: trade.symbol, id: trade.backendId })
      });
    } catch (e) {
      console.error('[Queries] Failed to sync delete with backend:', e);
    }
  }
...
/**
 * Clear all trade history and analytics tables to reset database.
 */
export async function wipeAllTrades() {
  await db.transaction('rw', [
    ...
    await db.edgeScoreHistory.clear();
    await db.edgeSnapshots.clear();
  });
