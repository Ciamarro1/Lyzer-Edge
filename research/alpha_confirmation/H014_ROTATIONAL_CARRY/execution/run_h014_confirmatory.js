 ...
 * 4. Executes AD007CarryEngine across 6 core assets with monthly Top 3 rotation.
...
import { AD007CarryEngine } from '../../../alpha_discovery/AD007/core/ad007_carry_engine.js';
import { runCalendarBlockBootstrap } from '../../../alpha_factory/core/inference_battery.js';

const rootDir = process.cwd();
const baseDir = path.resolve(rootDir, 'research/alpha_confirmation/H014_ROTATIONAL_CARRY');
...
async function main() {
  ...
  console.log(`Time In Market: ${simRes.timeInMarketPct.toFixed(1)}%`);
  ...
  fs.writeFileSync(path.join(resultsDir, 'H014_CONFIRMATORY_RESULTS.json'), JSON.stringify(resultsPayload, null, 2), 'utf8');

  // Generate Verdict Markdown
  const verdictMd = `# 🏛️ LYZER LABS — LAUDO DE AUDITORIA CONFIRMATÓRIA: HIPÓTESE H014
...
A hipótese **H014** (*Regime-Conditional & Rotational Cash-and-Carry Arbitrage - Top 3 Equal-Weighted*) foi testada sob protocolo estrito de hipótese unitária ($M = 1$) no conjunto de dados de Holdout Temporal Virgem cobrindo o período de **2025-01-01 a 2026-08-31** (${totalPeriods} períodos de 8 horas, 608 dias).
...
A hipótese **H014** superou **100% dos gates constitucionais**, comprovando que a dispersão transversal e a rotação mensal nos Top 3 ativos resolvem de forma definitiva a compressão macro de taxas de funding.
O alfa é declarado matematicamente robusto, imune a overfitting temporal e economicamente viável.
**Encaminhamento:** Elegível para a fase de especificação de arquitetura de execução e dimensionamento de risco (Risk Allocation & Portfolio Construction).` : 
...
`;
  ...
  fs.writeFileSync(path.join(resultsDir, 'H014_CONFIRMATORY_VERDICT.md'), verdictMd, 'utf8');
