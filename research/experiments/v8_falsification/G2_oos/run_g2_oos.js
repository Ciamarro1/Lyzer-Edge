...
console.log('================================================================');
console.log('🔬 LYZER EDGE — FALSIFICATION CAMPAIGN: GATE G2 TEMPORAL OOS (v2)');
console.log('Protocol: G2_TEMPORAL_OOS_PROTOCOL_v2 (Frozen)');
...
// 3. Exact Leap-Year Chronological Segmentation
const IS_START = 0;
...
console.log(`  Out-Of-Sample Period:  Bars ${OOS_START} -> ${OOS_END - 1} (${OOS_END - OOS_START} bars, 20 civil months)\n`);
  ...
  for (let t = startIdx + LOOKBACK; t < endIdx; t++) {
    ...
    if ((t - (startIdx + LOOKBACK)) % HORIZON === 0 && t + HORIZON <= endIdx) {
      ...
      if (sigDir !== 0) {
        ...
        const tradeNet = tradeGross - FIXED_FRICTION; // Exact 10 bps deduction
...
const summaryPayload = {
  ...
  protocolVersion: 'G2_TEMPORAL_OOS_PROTOCOL_v2',
...
**Document ID**: \`G2_TEMPORAL_OOS_REPORT_v2\`  
